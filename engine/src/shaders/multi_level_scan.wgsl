...
@group(0)
@binding(1)
var<storage, read> input_array: array<InputData>;

@group(0)
@binding(2)
var<storage, read_write> predicate_array: array<u32>;

@group(0)
@binding(3)
var<storage, read_write> count_array: array<u32>;
...
// Local x-dimension % sg_size == 0
// Workgroup_x stride x-dimension * sg_size
@compute
...
fn main(@builtin(local_invocation_id)    local_id: vec3<u32>,
	...
	// Global offset (assume sg_size == 32). TODO: handle other cases.
	let GLOBAL_OFFSET = 32u * 32u * work_group_id.x; 

	// The index of 32-block;
	let BLOCK_INDEX = global_id.x / sg_size;

	// The total sum of all bits in the block.
	var group_sum = 0u;

	// Ballot-mask for the block. On 32-bit, use only x-component. For 64-bit, use x and y components etc.
	var mask: vec4<u32> = vec4<u32>(0u, 0u, 0u, 0u);

	// Number of one bits in the mask;
        var one_bits: u32;

	for (var i=0u ; i < sg_size ; i++) {
	    
	    let input_data_index: u32 = GLOBAL_OFFSET + sg_size * i + sg_id;
	...
	// Store all predicates at the same time.
        predicate_array[work_group_id.x * sg_size + sg_id] = mask.x; 
	...
	//if (sg_id == 0u) {
	//    count_array[work_group_id.x] = group_sum;
	//}

	//////////////////////////////////////////////////////////////////////////

	//++ // One dispatch has max total number of elements 128 / sg_size * 1024 
	//++ // This means the stride between work_group is 127

	//++ let LOCAL_BLOCK_UNIT_INDEX = local_id.x / sg_size;
